...
url = 'https://arxiv.org/list/cs.AI/pastweek?show=164'
...
num = soup.select('#dlpage > ul > li > a')
...
try:
    ...
    sql = 'CREATE TABLE shuju(标题 TEXT,序号 VARCHAR(20),作者 TEXT,分类 TEXT,时间 VARCHAR(20))'
    cur.execute(sql)
    print('表格创建成功')
    sql = 'INSERT INTO shuju(标题, 序号, 作者, 分类, 时间) VALUES (%s,%s,%s,%s,%s)'
    values = (data1.pop(0).get_text(),data2.pop(0).get_text(),data3.pop(0).get_text(),data4.pop(0).get_text(),item5.get_text())
    db.commit()
    print('数据插入成功')
...
import requests
pdf_url='https://arxiv.org/pdf/'+'i2'
r = requests.get(pdf_url)
with open('D:\pythonProject8\Artificial Intelligence','wb') as f:
    f.write(r.content)